...
function renderMoreProjectsSection() {
    ...
    const orderedDetailProjects = getAllProjects().filter((project) => {
        if (!project || typeof project.projectUrl !== 'string') return false;
        return project.projectUrl.startsWith('project-');
    });

    if (!orderedDetailProjects.length) return;

    const currentIndex = orderedDetailProjects.findIndex((project) => project.projectUrl === currentFile);
    let moreProjects = [];

    if (currentIndex >= 0) {
        for (let i = 1; i < orderedDetailProjects.length; i += 1) {
            const nextIndex = (currentIndex + i) % orderedDetailProjects.length;
            moreProjects.push(orderedDetailProjects[nextIndex]);
        }
    } else {
        moreProjects = orderedDetailProjects.slice();
    }
